"""#98 動作確認用: Jamie商談候補提示メッセージを個人DMへ試験送信するスクリプト。

2026-08-19以降、本番も個人DM（JAMIE_CANDIDATE_DM_OWNER）宛になったため、実際にJamie
webhookが届けば自動でDMに来る。それでも実会議を録らずに「今どんなメッセージ・ボタンに
なっているか」を手元で確認したい場合に使う。既存の未割当インボックス（status='inbox'）
から1件を選んで実際の候補判定（_inbox_candidates、商談名のタイトル文字列一致）をかけ、
同じメッセージをDMに送る。ボタンは「候補（最大5件）」「対象（候補以外）」「対象外（割当不要）」
の3種類。ボタンのaction_id/valueは本番と同一実装を使うため、

    ⚠️ 「対象（候補以外）」ボタンは割当なしでWebインボックスへのリンクを返すだけ＝安全。
    ⚠️ 「対象外（割当不要）」ボタンを押すと、本当にstatus='not_needed'になり、
       見逃し検知・Web一覧から完全に除外される（元に戻せない）。押さずに見るだけならOK。
...
def main():
    ...
    try:
        ...
        if not candidates:
            print("[INFO] 候補0件（会議タイトルが商談名/アカウント名に一致しない）。"
                  "候補ボタンなし、「対象（候補以外）」「対象外（割当不要）」の2択のみで送信します。")
        ...
        if ts:
            ...
            print("     ⚠️ 「対象（候補以外）」ボタンは安全に押せます（Webへのリンクを返すのみ）。")
            print("     ⚠️ 「対象外（割当不要）」ボタンは押すとstatus='not_needed'になり元に戻せません。")
            print("     ⚠️ 商談名のボタンは押すと本当に割り当てが実行されます。")
        else:
