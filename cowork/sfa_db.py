...
# 提案未満（要件詰め/初回アポ実施/保留中）に差し戻された場合の区分。当初はバッジ表示のみを区別し、集計上の扱い
# (_DELIVERY_CONFIDENCE_BUCKET)・並び順(_DELIVERY_ACTIVE_CONF_RANK@webapp.py)・稼働集計除外は
# 「見込み(提案中)」と完全に同じ扱いだったが、Delivery一覧で「見込み(提案前)」が「見込み(提案中)」
# より上に表示されてしまう（開始週の早さだけで同一バケット内が並ぶため）不具合が報告されたため、
# 2026-09-24〜 並び順ランクだけ「見込み(提案中)」の直下に分離した（集計上の扱い・稼働集計除外は
# 従来通り「見込み(提案中)」と同じまま。バッジ色も変更なし）。
...
def delivery_performance_fee_total(dv: dict) -> float:
    """成果報酬の総額（想定インパクト×成果報酬比率÷100）。成果報酬有無='有'かつ想定インパクト・
    比率が両方設定されている場合のみ非ゼロ。固定報酬(fee_total/fee_monthly)とは別建てで、
    Delivery全体の報酬は固定報酬＋この値の合算になる（ユーザー要望2026-09-24: 固定報酬は
    従来通り入力し、そこに成果報酬を足し算する仕様。以前の「想定インパクト×比率が報酬額を
    置き換える」仕様から変更）。"""
    if not dv or dv.get("performance_fee") != "有":
        return 0.0
    impact, ratio = dv.get("expected_impact"), dv.get("performance_fee_ratio")
    try:
        if impact in (None, "") or ratio in (None, ""):
            return 0.0
        return round(float(impact) * float(ratio) / 100.0, 2)
    except (TypeError, ValueError):
        return 0.0
...
def delivery_weekly_productivity(con, delivery_id: int, weeks: list[str]) -> dict:
    ...
    全メンバー合算×その週の有効週数)は%週単位（例: 100%を12週続けると1200）で積み上がるため、
    月換算（既存のdelivery_month_count等と同じ1ヶ月=4週の換算）で÷4してから100%稼働あたりに
    割り戻す: 累計生産性 = Σ週別限界利益 × 400 ÷ Σ週別総稼働率。
    Delivery期間の前後に実稼働がある週（weeksが契約期間より広い＝delivery_gridがアサイン実働の
    範囲で返すため）は常に有効週数1.0でフル計上＝稼働だけを分母に加算し限界利益は0のまま
    （過大評価を防ぐ、既存方針）。
    ...
    成果報酬(performance_fee=='有'、想定インパクト×比率)は固定報酬(fee_total)とは別建てで、
    稼働のある最後の週(稼働最終週)にだけ全額を上乗せする（コスト削減案件等、成果報酬は稼働
    最終週に確定するため。固定報酬は従来通り週按分し、そこに成果報酬を足し算する仕様。
    ...
    # 稼働のある最後の週(稼働最終週)にだけ全額を上乗せする（按分せず、固定報酬との加算）。
    # 固定報酬は従来通り入力し、そこに成果報酬を足し算する仕様（ユーザー要望2026-09-24。
    # 以前の「想定インパクト×比率が報酬額を置き換える」仕様から変更）。
    performance_fee_total = delivery_performance_fee_total(dv)
    _staffed_weeks = [wk for wk in weeks if weekly_actual_total.get(wk, 0.0) > 0]
    _last_staffed_wk = max(_staffed_weeks) if _staffed_weeks else None
    ...
    for wk in weeks:
        ...
        if performance_fee_total and wk == _last_staffed_wk:
            rev += performance_fee_total
        ...
        weekly_cost[wk] = round(cost, 1)
        weekly_expense[wk] = round(exp, 1)
        weekly_margin[wk] = round(margin, 1)
        weekly_workload[wk] = round(work, 1)
        # ×400 = ÷4(%週→%月換算) ÷ (1/100)(100%稼働あたりに換算)。100%で4週(=1ヶ月)働けば
        # 月額報酬とちょうど一致する（自己整合性チェック。#189フォローアップ）。
        weekly_productivity[wk] = round(margin * 400 / work, 1) if work > 0 else None
        running_rev += rev
        running_cost += cost
        running_exp += exp
        ...
        cum_cost[wk] = round(running_cost, 1)
        cum_expense[wk] = round(running_exp, 1)
    ...
    return {"weeks": weeks, "fee_total": fee_total, "cost_total": cost_total, "expense_total": expense_total,
            ...
            "weekly_margin": weekly_margin, "cum_margin": cum_margin,
            "cum_workload": cum_workload, "productivity": productivity,
...
def compute_delivery_load(con, *, start_week: str | None = None,
    ...
    for r in con.execute(
        ...
        "dv.expected_impact AS expected_impact, dv.performance_fee_ratio AS performance_fee_ratio, "
        ...
        if r["delivery_id"] not in deliveries_meta:
            _dv_fee_like = {
                ...
                "expected_impact": r["expected_impact"], "performance_fee_ratio": r["performance_fee_ratio"],
            ...
            _cost_total = delivery_display_costs(_dv_fee_like)[1]
            deliveries_meta[r["delivery_id"]] = {
                ...
                "business_type_l1": _biz_l1 or "", "business_type_l2": _biz_l2 or "",
                "performance_fee_total": delivery_performance_fee_total(_dv_fee_like),
            }
