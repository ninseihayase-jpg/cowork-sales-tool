...
## 8. 一時停止（2026-08-17）— 再開ポイント

**ユーザー指示によりここで作業を一時停止。** ただし他の一時停止スレッド（#64/#97/#98）と違い、
本件は**システム側の実装は完全に完了しており、ブロッカーは無い**。唯一残っているのは
「本番DBへの`--apply`実行」というユーザー自身の運用判断のみ。

### 実装済み・push済み（コミット順）
1. `4709187` 本体修正: `sfa_db.complete_past_milestones`＋`add_activity`への組み込み、
   `slack_bot.apply_to_db`の生SQL撤廃（正本テーブル`deal_milestones`経由に統一）。
   バグ再現テスト含む6件追加、全243件パス。
2. `713d8ec` 既存データ補正: `scripts/backfill_stale_milestones.py`（dry-run既定・
   `--apply`で自動バックアップ後に適用）。テスト4件追加、全247件パス。

### 残っているのはユーザー操作のみ（ブロッカーではない）
- [ ] `python3 scripts/backfill_stale_milestones.py` で本番の滞留MS件数を確認
- [ ] 内容を見て問題なければ `--apply` で反映（実行前に自動バックアップされる）
- 上記はいつ実行しても良く、実行しなくても新規の書き込みは既に正しく動く
  （既存の滞留データがMS超過に出続けるだけで、新たな不整合は発生しない）。

### 次に着手すべきこと（再開時の入口）
1. 上記バックフィルの実行有無を判断・実施（急がなくてよい）。
2. §7の残課題（`set_deal_milestones`経路への自動補正拡張）に着手するかどうかを相談。
</content>
