...
## ステップ0: 直近2週間の一括バックフィル分析（§5.5・まずこれを実施）

日次のshadow運用（1日ずつ蓄積）に入る**前に**、鍵JSONができた時点でこちらを先に実行する。
Renderへの本設定前でも、鍵JSONさえあればローカルで実行できる。

```bash
GOOGLE_CALENDAR_SA_JSON=path/to/calendar-sa.json \
python scripts/calendar_crosscheck_backfill_report.py > backfill_report.txt
```

出力は6名それぞれの直近14日分の全予定を、外部会議候補と判定したかどうかのタグ付きで
人物ごとに通しで列挙したもの（Slackには一切投稿されない）。`backfill_report.txt`を
実際の顧客面談の記憶・記録と突き合わせ、「本当は顧客面談なのに`・対象外`タグになっている」
ケースを探す。見つかった癖は`DESIGN.md`§5.4に追記し、必要なら`cowork/workspace_calendar.py`の
`is_external_meeting`を調整してから、以下の日次shadow運用に進む。

## 動作確認・キャリブレーション（§5.4・日次運用）

バックフィル分析（ステップ0）を済ませたら、`CALENDAR_CROSSCHECK_MODE=shadow`のまま
さらに数日〜1-2週間運用し、**#salesへの投稿文面は一切変わらない**まま、毎朝早瀬個人の
Slack DMに「診断レポート」（6名それぞれのその日の全予定＋判定タグ）が届く。これを実際の
顧客面談と突き合わせ、判定漏れ（本当は顧客面談なのに「・対象外」タグになっている）が
無いかを継続確認する。
